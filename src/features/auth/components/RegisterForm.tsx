...
export const RegisterForm: React.FC = () => {
  const [form, setForm] = useState({ firstName: '', lastName: '', email: '', password: '', careerCode: '' });
    ...
    <form onSubmit={handleSubmit} className="space-y-4 w-full">
      ...
      <div className="w-full">
        ...
        >
          ...
          {careers?.map(c => (
            <option key={c.uvaCode} value={c.uvaCode}>{c.name}</option>
          ))}
        </select>
      </div>

      <Input label="Correo Institucional" type="email" value={form.email} onChange={e => setForm({...form, email: e.target.value})} placeholder="usuario@ucb.edu.bo" />
      ...
      <Button type="submit" className="w-full mt-4" isLoading={registerMutation.isPending}>Registrarme</Button>
    </form>
  );
};