...
export const ProgramList = () => {
  ...
  useEffect(() => {
    ...
    setIsModalOpen(params.get('create') === 'true');
  }, [location.search]);
    ...
    <div className="p-4 md:p-10 w-full max-w-7xl mx-auto text-white">
      <div className="flex flex-col md:flex-row md:justify-between md:items-end gap-6 mb-12">
        ...
        {(role === 'Coordinator' || role === 'SuperUser' || role === 'Admin') && (
            <Button variant="primary" className="!w-auto px-6 py-3 h-fit" onClick={() => navigate('/programs?create=true')}>
      ...
      {isLoading ? (
        ...
        <div className="text-zinc-500 text-center py-24 border-2 border-dashed border-zinc-900 rounded-3xl">
          <div className="max-w-xs mx-auto">
            ...
            <p className="text-sm">We couldn't find any programs at the moment. Try creating one!</p>
          </div>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-in fade-in duration-700">
          {programs.map((prog: any) => (
            <Card key={prog.id} className="group flex flex-col h-full p-8 hover:border-yellow-500/30 transition-all cursor-pointer bg-[#121214]" onClick={() => navigate(`/programs/${prog.id}`)}>
              ...
              <h2 className="text-xl font-bold mb-3 group-hover:text-yellow-500 transition-colors">{prog.name}</h2>
