...
import { Button, Card, StatusBadge, Alert, Tabs, SkeletonList } from '../../../components';
  ...
  if (!activity || error)
    return (
      <div className="p-20 text-center text-white">
        ...
        <Button onClick={() => navigate(-1)} variant="outline" className="!w-auto">
          Go Back
        </Button>
      </div>
    );
                    ...
                    <div className="flex items-center gap-4">
                      <div className="flex-1">
                        <p className="text-2xl font-bold text-white">
                          {activity.rule?.totalCapacity || 'Unlimited'}
                        </p>
                ...
                {!activity.rule?.groups || activity.rule.groups.length === 0 ? (
                  <div className="p-16 border border-dashed border-zinc-800 rounded-3xl text-center">
                    <p className="text-zinc-500">
                      No specific groups or shifts defined for this activity.
                    </p>
                  </div>
                ) : (
                  <div className="grid grid-cols-1 gap-4">
                    {activity.rule.groups.map((group) => (
                      <Card
                        key={group.uvaCode}
                        className="p-6 bg-[#121214] border-zinc-800/50 flex flex-col md:flex-row justify-between items-start md:items-center gap-6"
                      >
                        <div>
                          <h4 className="text-lg font-bold text-white mb-1">{group.name}</h4>
                          <p className="text-xs text-zinc-500 mb-4">
                            {group.details || 'No additional details.'}
                          </p>
                          <div className="flex gap-4 text-xs font-medium text-zinc-400">
                            ...
                            <span className="flex items-center gap-1.5">
                              <Clock size={14} />{' '}
                              {new Date(group.startDate).toLocaleTimeString([], {
                                hour: '2-digit',
                                minute: '2-digit',
                              })}
                            </span>
                ...
                <div className="space-y-6 text-sm text-zinc-400">
                  <div className="flex justify-between items-center py-3 border-b border-zinc-800/50">
                    ...
                    <span className="text-white font-mono">
                      {activity.rule?.registrationRadiusMeters || 0} meters
                    </span>
                  </div>
                  <div className="flex justify-between items-center py-3 border-b border-zinc-800/50">
                    <span>Enrollment Deadline</span>
                    <span className="text-white">
                      {activity.rule?.enrollmentDeadline
                        ? new Date(activity.rule.enrollmentDeadline).toLocaleString()
                        : 'No deadline'}
                    </span>
                  </div>
                  <div className="flex justify-between items-center py-3 border-b border-zinc-800/50">
                    ...
                    <span
                      className={`font-bold ${activity.rule?.requiresApproval ? 'text-yellow-500' : 'text-green-500'}`}
                    >
