...
import { LayoutDashboard, CalendarDays, Activity, History, PieChart, User, Settings, LogOut } from 'lucide-react';
...
export const Sidebar: React.FC = () => {
  ...
  const mainMenuItems = [
    ...
    { label: 'Historial', path: '/tracking', roles: ['Admin', 'SuperUser', 'Coordinator', 'Volunteer'], icon: History },
  ...
  const renderLinks = (items: typeof mainMenuItems) => (
    items.map((item) => {
      ...
      if (item.roles.length > 0) {
        return <RoleGuard key={item.path} requiredRoles={item.roles as any}>{link}</RoleGuard>;
      }
      return link;
    })
  );
